...
test.describe("Dashboard", () => {
  test.skip(
    ...
    "Requires TEST_EMAIL and TEST_PASSWORD for authenticated deck flows",
  );
  test("Generation, Study, and Deletion Flow", async ({ page }) => {
    await test.step("Navigate to Dashboard and check form elements", async () => {
      ...
      await expect(
        page.getByRole("heading", { name: "Generate a Deck" }),
      ).toBeVisible();
      await expect(page.getByText("Turn a topic or notes")).toBeVisible();
      ...
      await expect(
        page.getByRole("spinbutton", { name: "Cards" }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: "Generate Deck" }),
      ).toBeVisible();
    });
    ...
    await test.step("View My Decks", async () => {
      ...
      await expect(
        page.getByText("Study, favorite, and manage your"),
      ).toBeVisible();
